...
| BUG-ANDROID-KEYSTORE | On Android every `secure_*` call fails at runtime — _"No default store has been set, so cannot search or create entries"_ (logcat `E Tauri/Console`, seen on first launch from the startup Drive-connection check). Confirmed on device 2026-07-19 (M6 S1). | REQ-SEC-1 | 🔧 **in progress — fix landed in code (M6 S6a), awaiting on-device verification.** `keyring` v4 does **not** auto-register an Android store, so REQ-SEC-1 had no Android implementation even though it ships on Windows. **Fixed in S6a:** `android-native-keyring-store` + `keyring-core` added as `cfg(target_os="android")` deps, and the Keystore-encrypted store is registered as keyring-core's default in a `cfg(target_os="android")` `.setup()` hook. Init failure **logs to logcat and degrades** (never blocks app launch — local/SAF editing needs no keystore; `secure_*` then fail per-op as on desktop; hardened in the S6 adversarial review). Android build compiles; host tests can't exercise the JNI/Keystore tail. **Moves to ✅ once the on-device `secure_set→get→delete` round-trip (WF-33 Part A) passes.** Fallback if the crate misbehaves on device: `tauri-plugin-keyring`. |
