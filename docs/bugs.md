...
| Android SAF write (`BUG-SAF-WRITE-ATOMIC`, REQ-MOBILE-3) | `saf_write` is a truncate-and-replace against the `content://` document (`android-fs` `write()`), NOT the atomic sibling-temp+rename the local backend uses (`write_atomic`). An interrupted save (process kill / provider error / storage detached mid-write) can leave the file truncated. Found in the M6 S4 adversarial review, 2026-07-22. | SAF / scoped storage offers no portable atomic-rename across document providers, so an app-level atomic write would need a per-provider temp-document dance (create sibling doc → write → delete original → not even rename-able for most providers). Deferred: the window is narrow, most mobile editors accept it, and the conflict-detection rev already catches the *next* open. Revisit if data-loss reports appear. |

## Fixed

### M6 S6 (2026-07-25) — device-verified on a Pixel 9 Pro (Android 16)

| ID | Title | REQ | Notes |
|----|-------|-----|-------|
| BUG-ANDROID-KEYSTORE | On Android every `secure_*` call failed at runtime — _"No default store has been set, so cannot search or create entries"_ (logcat `E Tauri/Console`, on first launch from the startup Drive-connection check). Confirmed on device 2026-07-19 (M6 S1). | REQ-SEC-1 | ✅ **Fixed in M6 S6a — device-verified 2026-07-25 (WF-33 Part A).** `keyring` v4 ships no Android store, so REQ-SEC-1 had no Android impl even though it works on Windows. Fix has TWO parts: **(1)** register `android-native-keyring-store` as `keyring-core`'s default in a `cfg(android)` `.setup()` hook (`android-native-keyring-store` + `keyring-core` added as `cfg(android)` deps); **(2)** that crate gets its context from `ndk-context`, which **nothing in a Tauri app initializes** — tao (ndk-glue) never populates it and `tauri-plugin-android-fs` reaches its context another way — so `ndk_context::android_context()` **panicked** ("android context was not initialized") and *aborted app launch outright* on device. Fixed by calling the crate's JNI `initializeNdkContext(applicationContext)` from `MainActivity.onCreate` (a new `io.crates.keyring.Keyring` Kotlin shim; the symbol is in `libszmde_lib.so`) **before** the Rust runtime starts. The `.setup()` hook additionally wraps `Store::new()` in `catch_unwind` so a panic can never brick launch again (my first degrade only caught `Err`, not the panic — flagged low-sev in the S6 adversarial review, then proven on device). **Verified:** app boots with no rejection, and `secure_set→get→delete` round-trips through the AndroidKeyStore-encrypted store (`before=null → set → "roundtrip-OK" → delete → null`) driven over CDP. `secure_*`/`TauriSecureStore` unchanged; desktop untouched. |
