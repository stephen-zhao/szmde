...
| REQ-TBLED-7 | Edit-in-place — **click→exact char (S2):** a click inside any cell (incl. formatted `**x**`/`[t](u)`) maps to the source char via the shared `tokenizeInline` + `data-seg-from` segments (`renderedOffsetToSource`); fixes the M2 deferral (click landed at cell start). Arrows-enter-table is the remaining S2 piece | §7.4 | unit + integration (DOM) | `table-model.test.ts`, `table.dom.test.ts` |
