...
| REQ-TBLED-5 | Cursor-context shortcuts (move/insert/delete current row/col) — **commands + keybindings (S3):** `Mod-Enter`/`Mod-Shift-Enter` insert row below/above, `Alt-Shift-Arrows` move row/col (all return false off a table); delete + insert-col reachable via the gizmos/menu (S3b) | §7.4 | integration (DOM) | `table.dom.test.ts` |
| REQ-TBLED-6 | Auto-tidy + per-column alignment — **model (S1):** `parseTable`/`serialize`/`tidy`/`setColAlign` (columns from pipe geometry, empty cells preserved — fixes the lezer node-index bug; FITTED serialize, no alignment padding per the user); alignment UI + Tidy command in S4 | §7.4 | unit + integration (DOM) | `table-model.test.ts`, `table.dom.test.ts` |
| REQ-TBLED-7 | Edit-in-place (M5 S2): **click→exact char** inside any cell (incl. formatted `**x**`/`[t](u)`) via the shared `tokenizeInline` + per-segment `data-seg-from` (`renderedOffsetToSource`) — fixes the M2 deferral (click landed at cell start); **arrows enter a rendered table** (`enterTableDown`/`enterTableUp` reveal it instead of CM's atomic skip) | §7.4 | unit + integration (DOM) | `table-model.test.ts`, `table.dom.test.ts` |
