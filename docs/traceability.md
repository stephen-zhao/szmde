...
| REQ-RENDER-9 | Syntax mode (and Formatted reveal): a block marker's glyph run (`#`…/`>`) hangs in the LEFT gutter as an overhanging indent — an in-flow inline-block `Decoration.mark` whose `margin-left` is set to MINUS its own canvas-measured width (baked into the decoration's inline style, so CM re-applies it on every render and the marker never desyncs from the caret). Zero net inline advance (heading/quote text stays flush) without `position:absolute` (top-floats it) or `direction:rtl` (bidi-mirrors `>`); the trailing space stays an in-flow small-grey token. The chars stay REAL/EDITABLE/SELECTABLE in the document flow (a mark, never a replace/post-layout style) so the caret glides through them consistently; inline markers stay plain; no hang in Source | §4.1 | integration (DOM) | `editor/markers.dom.test.ts` (in-flow proof + cursor-glide contract: no widget-buffer, zero atomic, step-through positions, decoration-carried offset; live overhang/baseline/glide → WF-24) |
| REQ-RENDER-11 | Formatted-mode reveal-on-cursor renders markers in Syntax style (small-grey inline; hung block markers) via the shared `pushSyntaxStyle`, not raw Source literals; revealed markers stay editable (a mark, never atomic) and the text doesn't shift when the caret lands | §4.1 | integration (DOM) | `editor/markers.dom.test.ts` |
| REQ-FOLD-2 | The heading fold affordance is a prominent button chip (border + raised fill, `role=button` + `aria-expanded`), body-sized so it's gutter-safe on any heading level, consistent across all render modes | §5.4 | integration (DOM) + visual | `editor/fold.dom.test.ts` (button attrs; live prominence → WF-22) |
...
| REQ-RENDER-10 | Syntax/Formatted-reveal hung block markers are baseline-aligned with the heading/quote text | §4.1 | Pure CSS baseline (`vertical-align:baseline`) — happy-dom has no layout; covered by **WF-24**. |
