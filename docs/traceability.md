...
| REQ-RENDER-9 | Syntax mode (and Formatted reveal): block markers (`#`…/`>`) + trailing space hang in the LEFT gutter as an overhanging indent — an in-flow inline-block `Decoration.mark` whose OWN measured width is applied as a negative `margin-left` (the `hangMarkerMargins` measure plugin), so it overhangs left with zero inline advance (heading/quote text stays flush) without `position:absolute` (which top-floats it) or `direction:rtl` (which would bidi-mirror `>`); the chars stay REAL/EDITABLE/SELECTABLE in the document flow (a mark, never a replace widget); inline markers stay plain; no hang in Source | §4.1 | integration (DOM) | `editor/markers.dom.test.ts` (in-flow proof: no widget-buffer + zero atomic; live overhang/baseline → WF-24) |
