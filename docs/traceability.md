...
| REQ-RENDER-9 | Syntax mode (and Formatted reveal): block markers (`#`…/`>`) + trailing space hang in the LEFT margin as an overhanging indent (algorithmic — an in-flow `Decoration.mark` styled `display:inline-block; width:0; text-align:right`, each marker's own width sets the offset), so heading/quote text stays flush; the chars stay REAL/EDITABLE/SELECTABLE in the document flow (a mark, never a replace widget); inline markers stay plain; no hang in Source | §4.1 | integration (DOM) | `editor/markers.dom.test.ts` (in-flow proof: no widget-buffer + zero atomic; live overhang → WF-24) |
