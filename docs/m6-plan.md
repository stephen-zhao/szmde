...
2. ✅ **RESOLVED (S6a, device-verified 2026-07-25, Pixel 9 Pro — WF-33 Part A).** _Was: confirmed on
   device 2026-07-19, S1._ `keyring` v4 does **not** auto-register an Android store. The app boots fine, but any
   ...
   in a `cfg(target_os="android")` `.setup()` hook. **On-device this needed a second fix (2026-07-25):**
   the crate gets its context from `ndk-context`, which nothing in a Tauri app initializes, so
   `ndk_context::android_context()` panicked and aborted launch — fixed by calling the crate's JNI
   `initializeNdkContext(applicationContext)` from `MainActivity.onCreate`, and the `.setup()` hook now
   `catch_unwind`s `Store::new()` (a panic can never brick launch; init failure logs + degrades so
   `secure_*` fail per-op as on desktop). The `secure_set→get→delete` round-trip passed on a Pixel 9 Pro
   (WF-33 Part A) — see BUG-ANDROID-KEYSTORE. Fallback (unused) was `tauri-plugin-keyring`.
