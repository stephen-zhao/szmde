...
| **S4** 🔜 | SAF local storage backend (offline open/save) | REQ-MOBILE-3 | **Phase A device spike done 2026-07-21** — mechanism resolved (`tauri-plugin-android-fs`, decision #3) and the whole contract confirmed live on the Pixel 9 Pro (persist → force-stop+relaunch → standalone read; write updates the rev; risk #7 resolved). Remaining: `SafProvider` seam (offline TDD, id `"local"`) + `saf_*` Rust commands over android-fs's Rust API (picker/persist/read/write/metadata) + settings via app-private `std::fs`. **On-device acceptance: pick a real `.md`, edit, save back (with conflict detection), reopen after app restart via the persisted URI — fully offline.** The milestone's core shippable. |
