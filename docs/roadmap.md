...
| REQ-SCROLL-1 | **Typewriter scrolling — keep the active line vertically centred** (§4.5), on by default, with a settings flag to disable. CodeMirror's default `scrollIntoView` does the MINIMUM scroll, so a downward-moving caret rests one line inside the bottom edge; on a phone that is against the soft keyboard **and underneath the status chips**, so most of the line being edited is unreadable (measured on a physical Pixel 9 Pro: active line at y=578 in a 579px visible area). **General editor behavior on every platform**, not phone-only — just most acute there. Likely shape: a CodeMirror `EditorView.scrollMargins` facet (centrally reserves space for the ~14 existing `scrollIntoView: true` dispatch sites without touching them) and/or explicit `scrollIntoView(pos, {y:"center"})`; beware dispatch loops from an `updateListener`. Side effect: removes the status-chip overlap, so no chip hide/fade logic is needed. _(reported 2026-07-20 from on-device typing)_ | §4.5, §7.1 |
