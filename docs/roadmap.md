...
| **M6 — Android** (current milestone) | REQ-MOBILE-* | ✅ **local-first + Drive sign-in complete — S1–S6 merged, device-verified on a Pixel 9 Pro.** **S1** (cross-compile, all 4 ABIs + `gen/android`), **S2** (responsive shell), **S3** (soft-keyboard/IME inset bridge), **S4** (SAF storage, WF-32), **S5** (signed-release workflows + Android CI), **S6** (Google Drive sign-in — Android Keystore WF-33 Part A + a reverse-client-id **custom-scheme** OAuth redirect WF-33 Part B, both device-verified; an Android OAuth client rejects https App Link redirects). Remaining before a **shipped** build is maintainer-only: the S5 upload keystore + repo secrets + a `workflow_dispatch` dry-run, and a release Android OAuth client (release SHA-1 + "Custom URI scheme" + its manifest scheme). Follow-ons: **M6.1** (native Drive Picker for pre-existing files) · **M6.2** (touch-UX pass). [m6-plan.md](m6-plan.md) |
...
| REQ | Requirement | SPEC | Status |
|-----|-------------|------|--------|
| REQ-UI-4 | **Command reachability on touch** — every shipped command invocable without a keyboard, hover or right-click (starting with Find & Replace, which has no hamburger entry today) | §7 | ✅ built 2026-07-27 (WF-37) — Undo/Redo/Find/Bold/Italic → hamburger **Edit** section; audit routed the rest to REQ-UI-5 (Tab) + REQ-TBLED-9 (table structural) |
...
| REQ-LANE-1 | **Lane model + settings** — the left-edge columns become named lanes with a declared strategy (reserved / drawer / off); configured via an **ordered list of lane-id references** (spatial order) + a **per-lane settings object** (strategy, `drawerHeight` z-order) | §7.6 | ✅ built 2026-08-06 (unit) — `settings.lanes` model + bespoke validation + `applyLanes` CSS-var derivation (`off` reclaims width + hides affordance); `drawer` reserves width until REQ-LANE-4; defaults reproduce today's layout |
| REQ-LANE-4 | **Collapsible lane drawers** — `drawer`-strategy lanes collapse off the left and reveal on a left-edge swipe as **cascading, overlapping, finger-following** panels (leftmost slowest, rightmost fastest; content shifts right; `drawerHeight` = z-order; shadow/border delineation). Narrow auto-collapses; wide defaults open + toggle. **Partial-open peek persists** (type while half-out); interacting with a drawer affordance snaps it fully open. `reserved` lanes (e.g. line numbers) stay up. _(supersedes the removed REQ-LANE-2/3; interactive demonstrator built 2026-07-26)_ | §7.6 | 🟡 **static collapse built 2026-08-09** (WF-39) — per-breakpoint `defaultOpen` (wide-open / narrow-collapse, wide byte-identical), open scalar × width in `theme.ts`, `laneDrawers` rAF-tween + per-frame re-measure (caret-correct via genuine padding), 600px breakpoint + **View → Side lanes** toggle (survives file open). **Deferred:** the finger-follow `reveal` gesture (PR3) and the cascade (PR4 — buildable only as a decorative overlay on the real padding shift; real glyphs can't overtake without re-desyncing the caret, see SPEC §7.6) |
...
| REQ-UI-4 | ✅ **Built 2026-07-27** (M6.2, WF-37; tracked in requirements.md gaps). **Every command needs a pointer-agnostic entry point** (§7.1 "Command reachability"). **Verified gap:** Find & Replace (`REQ-FR-1`, shipped in M4) was bound *only* to `Mod-f` via `searchKeymap` with **no hamburger entry** — impossible to open on touch-only Android. **Done:** a hamburger **Edit** section (Undo / Redo / Find & Replace / Bold / Italic) driving `EditorApi.undo`/`redo`/`openFind`/`toggleBold`/`toggleItalic` — **Undo/Redo were surfaced by the pre-merge adversarial review** (Mod-z/Mod-y are equally unreachable on a soft keyboard). The audit routed the remaining keyboard-only paths to their own slices — **Tab** (indent / list-nest / next-cell) → `REQ-UI-5`, **table structural edits** → `REQ-TBLED-9`; zoom/page-width gestures (`Ctrl/Shift`-scroll) remain a coarse-pointer follow-up. _(found on-device 2026-07-20)_ | §7.1, §5.4 |
...
| REQ-INTEG-1 | **Windows Explorer "Open in szmde" context menu** for markdown files (`.md`/`.markdown`/…). Installer-registered shell verb / `IExplorerCommand` (the Win11 top-level menu needs `IExplorerCommand`; a classic verb falls under "Show more options") that launches szmde on the file via the §2.1 launcher path. Desktop-Windows; macOS Finder **Open With** is a sibling. _(requested 2026-07-26)_ | §2.2 |
| REQ-INTEG-2 | **"Open with szmde" from the Google Drive *web* UI** (drive.google.com). Needs a published Google Workspace Marketplace app with a *Drive UI integration* (MIME/ext + `open` URL) that hands the file id off to szmde (protocol/deep-link or web build); reuses the `drive.file` grant. **Larger / highest-uncertainty** — external Marketplace listing + Google review. _(requested 2026-07-26)_ | §2.2, §6 |
| REQ-INTEG-3 | **"Open in szmde" from the Google Drive *mobile app*.** szmde registers an Android intent-filter for the markdown MIME type; the Drive app's Open-with hands off the `content://` URI, which the M6 `SafProvider` already reads. Android-only, gated on the M6 app. _(requested 2026-07-26)_ | §2.2, §6 |
