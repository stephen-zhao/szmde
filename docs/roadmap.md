...
| **M6 — Android** (current milestone) | REQ-MOBILE-* | 🚧 in progress — **S1** (cross-compile, all 4 ABIs + `gen/android` committed), **S2** (responsive shell), **S3** (soft-keyboard/IME inset bridge) and **S4** (SAF storage — open/save real device files, verified on a Pixel 9 Pro) merged; **S5** (signed release + Android CI) workflows landed — pending the maintainer keystore/secrets + a dry-run for ✅. [m6-plan.md](m6-plan.md) |
