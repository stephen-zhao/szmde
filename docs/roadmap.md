...
| **M6 — Android** (current milestone) | REQ-MOBILE-* | 🚧 in progress — **S1** (cross-compile, all 4 ABIs + `gen/android` committed), **S2** (responsive shell), **S3** (soft-keyboard/IME inset bridge) and **S4** (SAF storage — open/save real device files, verified on a Pixel 9 Pro) merged; **S5** (signed release + Android CI) merged — pending the maintainer keystore/secrets + a dry-run for ✅; **S6** (Drive sign-in — scoped to sign-in; Picker → M6.1): the **Android Keystore is device-verified** (WF-33 Part A, Pixel 9 Pro); the deep-link OAuth flow (Part B) is code-complete offline, pending the maintainer prereqs (hosted assetlinks.json + a separate Android OAuth client). [m6-plan.md](m6-plan.md) |
